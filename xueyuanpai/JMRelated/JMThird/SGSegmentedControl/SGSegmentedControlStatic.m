...
#define indicatorViewWidth 20
#define indicatorViewDistanceToBottom 5
...
- (SGImageButton_Horizontal *)imageTitle_btnH
{
    if (!_imageTitle_btnH) {
        ...
        // 计算Titlebutton内容的Size
//        CGSize buttonSize = [self sizeWithText:firstButton.title font:VMUIFont(SG_defaultTitleFont) maxSize:CGSizeMake(MAXFLOAT, self.frame.size.height)];
...
- (void)selectedBtnLocation:(UIButton *)button
{
    ...
    [UIView animateWithDuration:SG_indicatorAnimationTime animations:^{
        if (self.isFull) {
            self.indicatorView.SG_width = indicatorViewWidth; //self.frame.size.width / _title_Arr.count;
        } else {
            if (_isButtonH) {
                self.indicatorView.SG_width = indicatorViewWidth; //buttonHSize.width + SG_defaultHorizontalImageWidth + 5;
            } else if (_isButtonV) {
                self.indicatorView.SG_width = indicatorViewWidth; //buttonVSize.width;
            } else {
                self.indicatorView.SG_width = indicatorViewWidth; //buttonSize.width;
            }
        }
