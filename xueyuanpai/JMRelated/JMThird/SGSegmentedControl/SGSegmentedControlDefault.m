...
#pragma mark - - - 为文字缩放增加的代码
    ...
    _indicatorView.SG_y = self.frame.size.height - SG_indicatorHeight - indicatorViewDistanceToBottom;
    [self addSubview:_indicatorView];

    // 指示器默认在第一个选中位置
    // 计算Titlebutton内容的Size
//    CGSize buttonSize = [self sizeWithText:firstButton.titleLabel.text font:(VMUIFont(SG_defaultTitleFont)) maxSize:CGSizeMake(MAXFLOAT, self.frame.size.height)];
#pragma mark - - - 判断是否开启文字缩放功能
    if (self.isScaleText) {
        _indicatorView.SG_width = indicatorViewWidth; //buttonSize.width + SG_btnScale * buttonSize.width;
    } else {
        _indicatorView.SG_width = indicatorViewWidth; //buttonSize.width;
    }
...
{
    [self clickTitleBtnAction:sender];
}
...
- (void)selectedBtnLocation:(UIButton *)button
{
    ...
    if (self.segmentedControlIndicatorType == segmentedControlIndicatorTypeCover) {
        [UIView animateWithDuration:SG_indicatorAnimationTime animations:^{
            weakSelf.indicatorView.SG_width = indicatorViewWidth; //button.SG_width - SG_btnMargin;
            weakSelf.indicatorView.SG_centerX = button.SG_centerX;
        }];

    } else if (self.segmentedControlIndicatorType == segmentedControlIndicatorTypeImageView) {
        [UIView animateWithDuration:SG_indicatorAnimationTime animations:^{
            weakSelf.indicatorViewWithImage.SG_centerX = button.SG_centerX;
        }];

    } else {
        // 改变指示器位置
        [UIView animateWithDuration:SG_indicatorAnimationTime animations:^{
            weakSelf.indicatorView.SG_width = indicatorViewWidth; //button.SG_width - 2 * SG_btnMargin;
            weakSelf.indicatorView.SG_centerX = button.SG_centerX;
        }];
    }
...
- (void)setSegmentedControlIndicatorType:(segmentedControlIndicatorType)segmentedControlIndicatorType
{
    ...
    if (self.segmentedControlIndicatorType == segmentedControlIndicatorTypeDefault) {
        ...
        // 改变原指示器样式
        _indicatorView.SG_width = indicatorViewWidth; //buttonSize.width + SG_btnMargin;
        _indicatorView.SG_centerX = firstButton.SG_centerX;
