...
#define SendButtonColor [UIColor colorWithRed:18/255.0 green:204/255.0 blue:153/255.0 alpha:1.0]
...
-(void) initView
{
    ...
    if (_inputTextView == nil) {
        ...
        UIView *inputBackGroundView = [[UIView alloc] initWithFrame:CGRectMake(x, y, width, height)];
        inputBackGroundView.layer.borderWidth = 0.5;
        inputBackGroundView.layer.cornerRadius = 5;
        inputBackGroundView.backgroundColor = [CommonUtils colorWithHex:@"FCFCFC"];
        inputBackGroundView.layer.borderColor = [CommonUtils colorWithHex:@"ECECEC"].CGColor;
        [_inputView addSubview:inputBackGroundView];
        
        
        _inputTextView = [[UITextField alloc] initWithFrame:CGRectMake(6, 0, width, height)];
        [inputBackGroundView addSubview:_inputTextView];
        
        _inputTextView.keyboardType = UIKeyboardTypeDefault;
        _inputTextView.returnKeyType = UIReturnKeyDone;
        _inputTextView.backgroundColor = [CommonUtils colorWithHex:@"FCFCFC"];
...

{
    ...
    // 解决搜狗键盘缩回的bug
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(hideInputView) name:UIKeyboardWillHideNotification object:nil];
}
