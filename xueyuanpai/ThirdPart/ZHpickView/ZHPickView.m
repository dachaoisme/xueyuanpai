...
-(void)doneClick
{
    if (_pickerView) {
        ...
        _resultString=[self stringFromDate:_datePicker.date];//[NSString stringWithFormat:@"%@",_datePicker.date];
    }
    if ([self.delegate respondsToSelector:@selector(toobarDonBtnHaveClick:resultString: withDate:)]) {
        [self.delegate toobarDonBtnHaveClick:self resultString:_resultString withDate:_datePicker.date];
    }
    [self removeFromSuperview];
}
